...
<script>
   $(document).ready(function() {
      ...
      $(document).on("click","#minus_btn",function(){
         ...
         if( minusNum < 1){
            ...
            if( item_amount*minusNum < item_free){
               ...
               $(this).parent().parent().parent().parent().find('.itemchBox').data("item_shipping",item_basic);
            }else{
               $(this).parent().parent().parent().next().next().text("무료배송");
               $(this).parent().parent().parent().parent().find('.itemchBox').data("item_shipping",0);
            }
      ...
      function totalshipping(){
         var shipsum=0;
         var chnum=$("input:checkbox[name=cart_no]:checked").length;
         
         //체크 안됐을 때 = 0원
          if(chnum==0){
             $('#total_shipping').text(0+"원");
          }else{
           //체크 됐을 때
          $.each($("input:checkbox[name=cart_no]:checked"), function( key, value ) {
             var item_amount=$(this).data("item_amount");
             var item_free=$(this).data("item_free");
             var fix_shipping=$(this).data("fix_shipping");
             var seller_code = $(this).data("seller_code");
                
                //무료배송 조건 충족
                if( item_amount >= item_free){
                   $(this).data("item_shipping",0);
                   //한개가 무료배송이면 다 무료배송
                   $.each($("input:checkbox[data-seller_code="+seller_code+"]"), function( key, value ) {
                      $(this).data("item_shipping",0);                      
                      })
                  } else { //무료배송 조건 미충족 
                     $(this).data("item_shipping",fix_shipping);
                  }
            });
         ...
         var beforeseller="";           
             $.each($("input:checkbox[name=cart_no]:checked"), function( key, value ) {
             var ship=$(this).data("item_shipping"); //선택된 상품 배송비
             if(ship>0){
             if(beforeseller != $(this).data("seller_code")){
                shipsum += ship;
             }
             }
             beforeseller = $(this).data("seller_code");
             $('#total_shipping').text(shipsum+"원");
             })
         }
       };
      ...
      // 전체 합계 - 전체 주문금액
       function finalprice(){
          var total_price=$('#total_price').text(); //2000원
          var total_shipping=$('#total_shipping').text();
          $('#final_price').text(Number(total_price.split("원",1))+Number(total_shipping.split("원",1))+"원");
      ...
      <div id="content" class="mx-auto">
         ...
         <div class="shopping-cart section">
            ...
            <div class="container">
               ...
               <div>
                  <h1 class="chartheader mt-3 mb-5">장바구니</h1>
               </div>
               <div class="row">
                  <div class="col-12">
                     ...
                     <table class="table shopping-summery">
                        <tbody>
                           ...
                           <c:forEach var="sellerList" items="${sellerList}" >
                              <tr class="seller">
                                    <td colspan="7" class="sellerinfo px-2 bg-">
                                       <div class="custom-control custom-checkbox">
                                    ...
                                    <td class="image px-3" data-title="No">
                                       <div class="custom-control custom-checkbox">
                                          <input type="checkbox" class="chBox itemchBox custom-control-input"
                                             ...
                                             data-item_shipping="${cartList.basic_shipping_cost}"
                                             data-fix_shipping="${cartList.basic_shipping_cost}"
                                             data-item_free="${cartList.free_shipping_cost}">
                           ...
                           <div class="col-lg-10">
                              <div class="right">
                                 <ul>
                                    ...
                                    <li>상품금액<span id="total_price"></span></li>
                                    <li>배송비<span id="total_shipping"></span></li>
                                    <li class="last">전체 주문금액<span id="final_price"></span></li>
                                 </ul>
                              </div>
                           </div>
